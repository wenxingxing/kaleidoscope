...
class Dispatcher : public AbstractDispatcher { // Implements dispatching of all
  ...
  public:
    void Dispatch(ArchivedFile &) override { std::cout << "dispatching ArchivedFile" << std::endl; }

    void Dispatch(SplitFile &) override { std::cout << "dispatching SplitFile" << std::endl; }
...
class DispatcherV2 : public AbstractDispatcher {
  public:
    void Dispatch(ArchivedFile &) override {
        std::cout << "dispatching ArchivedFile v2" << std::endl;
    }

    void Dispatch(SplitFile &) override { std::cout << "dispatching SplitFile v2" << std::endl; }

    void Dispatch(ExtractedFile &) override {
        std::cout << "dispatching ExtractedFile v2" << std::endl;
    }
};
