...
static int gettok() {
    ...
    if (isalpha(LastChar)) {
        ...
        while (isalnum((LastChar == getchar()))) IdentifierStr += static_cast<char>(LastChar);
...
class BinaryExprAST : public ExprAST {
   ...
   public:
    ...
    std::string str() override { return string("(") + Op + " " + LHS->str() + " " + RHS->str() + ")"; }
};
...
// GetTokPrecedence - Get the precedence of the pending operator token.
...
static std::unique_ptr<ExprAST> ParseBinOpRHS(int ExprPrec, std::unique_ptr<ExprAST> LHS) {
    ...
    while (true) {
        ...
        if (TokPrec < NextPrec) {
            RHS = ParseBinOpRHS(TokPrec + 1, std::move(RHS));  // todo: +1??
            if (!RHS) return nullptr;
        }
        // Merge LHS/RHS
